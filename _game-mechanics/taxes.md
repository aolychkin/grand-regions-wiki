...
> Status: 🟢 базовый сбор реализован; 🟡 двухуровневое распределение, POST-MVP.

## Принцип

Каждое здание платит налог с прибыли. Куда идёт налог, зависит от **владельца здания**:

| Владелец здания (владелец здания) | % в город | % в страну | % владельцу |
|---|---|---|---|
| **Частное** (игрок-владелец) | 15% | 5% | 80% (игроку) |
| **Муниципальное** (мэрия) | 0%* | 10% | 90% (в бюджет города) |
| **Государственное** (государство) \[v2\] | 0% | 0% | 100% (в бюджет страны) |

\* Город не платит налог сам себе, это было бы бессмыслицей.

> Default-числа выше, гипотезы. Точные ставки устанавливаются Госдумой (лимиты) и Гордумой (финал в лимитах). См. .
...
|---|---|---|---|---|
| **Подоходный** | ЗП NPS и Player | Город | 0...20% | бюджет города |
| **Налог на прибыль фабрик** | Доход частных заводов | Город (15%) + Страна (5%) | 0-20% / 0-10% | См. таблицу выше |
| **Налог с продаж** | Розница | Город | 0...2% | бюджет города |
| **Импортные пошлины** | Покупки извне | Город | 0...10% | бюджет города |
| **Налог на землю** | Владение Territory | Регион/Город | 0...5% | бюджет региона / бюджет города |
| **Лицензионный сбор** | Открытие пивоварни и т.п. | Город (разовый) | По таблице | бюджет города |

## Что происходит когда страна нейтральна

Если на старте мира **никто не основал государство**, налоговая часть «в страну» (5% или 10%) не пропадает, она **копится на счёте Country** (бюджет страны). Когда первая партия захватывает регион и основывает государство → все накопленные деньги переходят к новому Президенту.
...
Все транзакции пишутся в журнал налогов с полями: откуда (здание), куда (город / страна / игрок), сумма, тип налога, время тика. Каждый тик считает прибыль каждого здания, применяет таблицу выше, делает атомарную транзакцию по трём направлениям.
...
См. [Ресурсы и региональные резервы](resources-and-reserves.md) для деталей.
