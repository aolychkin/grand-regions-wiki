...
> Status: 🟡 , POST-MVP. Backbone для прокачки игрока.
...
Strength  = 10
Endurance = 10
Active = NULL  ← выбираешь сам что качать
```
...
Building тип `school` / `academy`. Платишь курс → +1 к навыку за **7 игр. дней**.
...
Микротранзакция: **100 GP = +1 к навыку**, мгновенно. Ограничение из anti-P2W cap:
...
В **«Профиль»** → Навыки ты выбираешь `active_skill ∈ {knowledge, strength, endurance}`. **Все** XP (от работы, школы, GP) идут в этот навык. Остальные два не растут.
